import {
  ...
  wrapInList,
  zt
} from "./chunk-3UNQT4JR.js";
import {
  useConvex,
  useMutation,
  useQuery
} from "./chunk-PFGDO4EM.js";
import "./chunk-QYNTPBMV.js";
import {
  require_react
...
// node_modules/@convex-dev/prosemirror-sync/dist/esm/blocknote/index.js
...
// node_modules/@convex-dev/prosemirror-sync/dist/esm/tiptap/index.js
var import_react2 = __toESM(require_react(), 1);
var MAX_STEPS_SYNC = 1e3;
var SNAPSHOT_DEBOUNCE_MS = 1e3;
function useTiptapSync(syncApi, id, opts) {
  const log = (opts == null ? void 0 : opts.debug) ? console.debug : () => {
  };
  ...
  const submitSnapshot = useMutation(syncApi.submitSnapshot).withOptimisticUpdate((localQueryStore, args) => {
    ...
    if (!(existing == null ? void 0 : existing.content)) {
  ...
  const create = (0, import_react2.useCallback)(async (content) => {
...
function syncExtension(convex, id, syncApi, initialState, opts) {
  const log = (opts == null ? void 0 : opts.debug) ? console.debug : () => {
  };
  let snapshotTimer;
  const trySubmitSnapshot = (version, content) => {
    ...
    snapshotTimer = setTimeout(() => {
      void convex.mutation(syncApi.submitSnapshot, { id, version, content }).catch(opts == null ? void 0 : opts.onSyncError);
    }, (opts == null ? void 0 : opts.snapshotDebounceMs) ?? SNAPSHOT_DEBOUNCE_MS);
  };
  let active = false;
  let pending;
  let watch;
  async function trySync(editor) {
    const serverVersion = watch == null ? void 0 : watch.localQueryResult();
    if (serverVersion === void 0) {
      return;
    }
    if (serverVersion && serverVersion > getVersion(editor.state)) {
      clearTimeout(snapshotTimer);
    ...
    active = true;
    try {
      if (await doSync(editor, convex, syncApi, id, serverVersion, initialState, opts == null ? void 0 : opts.debug)) {
      ...
      if (opts == null ? void 0 : opts.onSyncError) {
  ...
  return Extension.create({
    ...
    onCreate() {
      var _a;
      if ((_a = initialState.restoredSteps) == null ? void 0 : _a.length) {
...
async function doSync(editor, convex, syncApi, id, serverVersion, initialState, debug) {
  ...
  while (true) {
    ...
    const steps = sendable.steps.slice(0, MAX_STEPS_SYNC).map((step) => JSON.stringify(step.toJSON()));
    ...
    if (result.status === "synced") {
      ...
      log("Synced", {
        steps,
        version,
        newVersion: getVersion(editor.state)
      });
      continue;
    }
...
function receiveSteps(editor, steps, clientIds) {
  editor.view.dispatch(receiveTransaction(editor.state, steps, clientIds, {
    mapSelectionBackward: true
  }));
}
function useInitialState(syncApi, id, cacheKeyPrefix) {
  const serverRef = (0, import_react2.useRef)({ id });
  const cachedState = (0, import_react2.useMemo)(() => {
  ...
  const snapshot = (0, import_react2.useMemo)(() => {
    return serverInitial && serverInitial.content !== null && {
      initialContent: JSON.parse(serverInitial.content),
      initialVersion: serverInitial.version
    };
  }, [serverInitial]);
  if (snapshot || serverRef.current.id !== id) {
    serverRef.current = { id, snapshot: snapshot || void 0 };
  }
  ...
  if (!cachedState && (serverInitial == null ? void 0 : serverInitial.content) === null) {
    return {
      loading: false,
      initialContent: null
    };
...
// node_modules/@convex-dev/prosemirror-sync/dist/esm/blocknote/index.js
function useBlockNoteSync(syncApi, id, opts) {
  const sync = useTiptapSync(syncApi, id, opts);
  const editor = (0, import_react3.useMemo)(() => {
    var _a, _b, _c;
    if (sync.initialContent === null)
      return null;
    const editor2 = zt.create({
      ...opts == null ? void 0 : opts.editorOptions,
      _headless: true
    });
    const blocks = [];
    const pmNode = editor2.pmSchema.nodeFromJSON(sync.initialContent);
    if (pmNode.firstChild) {
      pmNode.firstChild.descendants((node) => {
        blocks.push(X(node, editor2.pmSchema));
        return false;
      });
    }
    return zt.create({
      ...opts == null ? void 0 : opts.editorOptions,
      _tiptapOptions: {
        ...(_a = opts == null ? void 0 : opts.editorOptions) == null ? void 0 : _a._tiptapOptions,
        extensions: [
          ...((_c = (_b = opts == null ? void 0 : opts.editorOptions) == null ? void 0 : _b._tiptapOptions) == null ? void 0 : _c.extensions) ?? [],
          sync.extension
        ]
      },
      initialContent: blocks.length > 0 ? blocks : void 0
    });
  }, [sync.initialContent]);
  if (sync.isLoading) {
    return {
...
//# sourceMappingURL=@convex-dev_prosemirror-sync_blocknote.js.map
