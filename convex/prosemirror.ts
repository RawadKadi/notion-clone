...
async function checkPermissions(ctx: GenericQueryCtx<DataModel>, id: string) {
  ...
  if ('isPublic' in document && 'createdBy' in document) {
    // Allow access if document is public or user is the creator
...
async function checkWritePermissions(ctx: GenericMutationCtx<DataModel>, id: string) {
  ...
  // Check if this is a documents table entry
  if ('isPublic' in document && 'createdBy' in document) {
    // Only allow editing if document is public or user is the creator
    if (!document.isPublic && document.createdBy !== userId) {
...
export const { getSnapshot, submitSnapshot, latestVersion, getSteps, submitSteps } = prosemirrorSync.syncApi<DataModel>({
  ...
  onSnapshot: async (ctx, id, snapshot, version) => {
    // Update the document's lastModified timestamp when content changes
    const document = await ctx.db.get(id as any);
    if (document && 'lastModified' in document) {
      await ctx.db.patch(id as any, { lastModified: Date.now() });
    }
  },
});
